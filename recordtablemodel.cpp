 ...
 * 更新记录
 */
void RecordTableModel::updateRecord(int row,QString &tagId, QString &time, QString status)
{
 ...
 * 添加记录
 */
int RecordTableModel::addRecord(QString &tagId, QString &time, QString status)
{
    QSqlRecord record;//也可以直接使用sql语句进行,但是sql语句更容易写错
    record.append(QSqlField(header.at(0), QVariant::String));
    record.append(QSqlField(header.at(1), QVariant::String));
