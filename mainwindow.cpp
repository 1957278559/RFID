...
{    
    ...
    if(m125dll->LF125K_FrameAnalysis((uint8 *)(data.data())) == 0)
    {
        ...
        int index = model->findRecord(tagId);//在表中查找该卡号所在行，不存在则新增一条记录，存在则修改记录
        if(index >= 0 )
        {
            QString text = model->record(index).value(2).toString();//获得进出状态
            if(text == tr("进")){
...
/**
 * @brief MainWindow::on_btn_clear_clicked
 * 清空当前表中数据(会删除数据库中所有的数据)
 */
